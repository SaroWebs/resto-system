...
const DropdownNotification = () => {
    ...
    let notifications = [
        ...
        },
        {
            url: '/test',
            title: 'Order Placed',
            description: 'An order (ORD001234) has been placed by Mr. test user',
            date: '12 May, 2024'
        }
    ];
    ...
    return (
        <li className="relative">
            ...
            <div
                ...
                className={`absolute -right-27 mt-2 flex h-90 w-75 flex-col rounded-sm border border-stroke bg-white shadow-default sm:right-0 sm:w-80 ${dropdownOpen === true ? 'block' : 'hidden'
                    }`}
            >
                ...
                <ul className="flex h-auto flex-col overflow-y-auto">
                    {notifications.length < 0 ? notifications.map((notification, i) => (
                        <li key={i}>
                            ...
                            >
                                <p className="text-sm">
                                    <span className="text-black font-bold mr-2">{notification.title}</span>
                                    <span className="text-gray-700">{notification.description}</span>
                                </p>
                        ...
                        <li className='px-4 py-3 text-xs text-red-400 w-full text-center'>No Notification found</li>
                    )}
                </ul>
            </div>
        </li>
